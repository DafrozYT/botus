...
**👉 [НАЧАТЬ УСТАНОВКУ](GETTING_STARTED.md) 👈**

Подробные инструкции для разных сценариев:

- **🖥️ [Установка на чистый VDS](VDS_INSTALL.md)** - Полное руководство с нуля
...
### 🔧 Автоматическая установка VDS

Для автоматической настройки чистого VDS сервера:

```bash
# Скачайте и запустите скрипт автонастройки
